...
<div class="max-w-4xl mx-auto mt-10 p-6 bg-white rounded shadow" data-aos="fade-up">
    ...
    <form action="{{ route('admin.users.update', $user->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <input type="datetime-local" name="email_verified_at" class="w-full border rounded px-3 py-2" value="{{ \Carbon\Carbon::parse($user->email_verified_at)->format('Y-m-d\TH:i') }}">
        </div>

        <div class="mb-4">
            <label class="block mb-1">Password (leave empty to keep current)</label>
            <input type="password" name="password" class="w-full border rounded px-3 py-2">
        </div>

        <div class="mb-4">
            <label class="block mb-1">Phone</label>
            <input type="text" name="phone" class="w-full border rounded px-3 py-2" value="{{ $user->phone }}">
        </div>

        <div class="mb-4">
            <label class="block mb-1">Avatar</label>
            @if($user->avatar)
                <img src="{{ asset('storage/' . $user->avatar) }}" alt="avatar" class="w-16 h-16 rounded mb-2">
            @endif
        ...
        <div class="mb-4">
            ...
            <div class="flex items-center gap-6">
                <label class="flex items-center gap-2">
                    <input type="radio" name="status" value="active" class="accent-green-600" {{ $user->status === 'active' ? 'checked' : '' }}>
                    <span>Active</span>
                </label>
                <label class="flex items-center gap-2">
                    <input type="radio" name="status" value="suspended" class="accent-red-600" {{ $user->status === 'suspended' ? 'checked' : '' }}>
                    <span>Suspended</span>
                </label>
            </div>
        </div>
